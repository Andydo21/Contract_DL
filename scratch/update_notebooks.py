...
# 1. Modify workflow-recommendation.ipynb to save the classification model at the end
rec_nb_path = "workflow-recommendation.ipynb"
if os.path.exists(rec_nb_path):
    with open(rec_nb_path, "r", encoding="utf-8") as f:
        rec_nb = json.load(f)
    
    # Check if we already appended the save cell
    already_saved = any(
        "workflow-recommendation-model" in "".join(c.get("source", []))
        for c in rec_nb["cells"]
    )
    if not already_saved:
        new_cell = {
            ...
            "outputs": [],
            "source": [
                "# Save the trained classification model for inference\n",
                "trainer.save_model(\"/kaggle/working/workflow-recommendation-model\")\n",
                "tokenizer.save_pretrained(\"/kaggle/working/workflow-recommendation-model\")\n",
                "print(\"Model saved to /kaggle/working/workflow-recommendation-model\")"
            ]
        }
        rec_nb["cells"].append(new_cell)
        with open(rec_nb_path, "w", encoding="utf-8") as f:
            json.dump(rec_nb, f, indent=1)
        print("Updated workflow-recommendation.ipynb successfully.")
else:
    print("workflow-recommendation.ipynb not found.")

# 2. Modify dynamic-workflow-builder.ipynb to save Flan-T5 and append FastAPI server
builder_nb_path = "dynamic-workflow-builder.ipynb"
if os.path.exists(builder_nb_path):
    with open(builder_nb_path, "r", encoding="utf-8") as f:
        builder_nb = json.load(f)

    # Clean up existing server cell if any to allow updating
    builder_nb["cells"] = [
        c for c in builder_nb["cells"]
        if not ("recommend_workflow_api" in "".join(c.get("source", [])))
    ]
    ...
    if True:
        ...
        builder_nb["cells"].append(new_cell)
        with open(builder_nb_path, "w", encoding="utf-8") as f:
            json.dump(builder_nb, f, indent=1)
        print("Updated dynamic-workflow-builder.ipynb successfully.")
else:
    print("dynamic-workflow-builder.ipynb not found.")
