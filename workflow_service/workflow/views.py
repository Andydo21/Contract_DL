...
def create_workflow(request):
    ...
    return JsonResponse({
        ...
        "confidence": getattr(workflow, "confidence", 0.0),
...
@csrf_exempt
def recommend_workflow_view(request):
    """POST /workflows/recommend/ — preview AI/rule-based recommendation without saving."""
    if request.method != "POST":
        return JsonResponse({"error": "Method not allowed"}, status=405)
    try:
        body = json.loads(request.body)
    ...
    workflow_type, steps, reasons, _, confidence = recommend_workflow(
        body.get("contract_text", ""),
        body.get("clause_types", []),
        body.get("contract_type", ""),
    )
    return JsonResponse({
        "workflow_type": workflow_type,
        "confidence": confidence,
        "steps": [
            {"step_order": i + 1, "step_name": step if isinstance(step, str) else step.get("step_name")}
